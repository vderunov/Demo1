...
export default function makeTestsForTask4(assert) {
  describe('Palindrome', () => {
    ...
    describe('Error conditions', () => {
      ...
      it('The letter at the beginning of the number: (a2546347)', () => {
        assert.deepEqual(checkParamTask4('a2546347'), {
          status: 'failed',
          reason: 'Enter only numbers',
        });
      });

      it('The letter in the middle of the number: (254a6347)', () => {
        assert.deepEqual(checkParamTask4('254a6347'), {
          status: 'failed',
          reason: 'Enter only numbers',
        });
      });

      it('Letter at the end of the number: (2546347a)', () => {
        assert.deepEqual(checkParamTask4('2546347a'), {
          status: 'failed',
          reason: 'Enter only numbers',
        });
      });

      it('Parameter not numbers: (!@#%$Homer)', () => {
        assert.deepEqual(checkParamTask4('!@#%$Homer'), {
          status: 'failed',
          reason: 'Enter only numbers',
        });
      });

      it('Parameter not numbers: (` `)', () => {
        assert.deepEqual(checkParamTask4(' '), {
          status: 'failed',
