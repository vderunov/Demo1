...
const test6 = function makeTestsForTask6(assert) {
  describe('Numeric sequence', () => {
    ...
    const valid = [
      {
        description:
          'Provided: length: 5, min square of number 36, result: "6, 7, 8, 9, 10"',
        arguments: [5, 36],
        result: '6, 7, 8, 9, 10',
      },
      {
        description:
          'Provided: length: 3, min square of number 29, result: "6, 7, 8"',
        arguments: [3, 29],
        result: '6, 7, 8',
      },
      {
        description:
          'Provided: length: 1, min square of number 110, result: "11"',
        arguments: [1, 110],
        result: '11',
      },
      {
        description:
          'Provided: length: 1, min square of number 49, result: "7"',
        arguments: [1, 49],
        result: '7',
      },
    ];
    ...
    valid.forEach(el => {
      it(el.description, () => {
        assert.equal(numericSequence(...el.arguments), el.result);
      });
    });
    ...
    describe('Error conditions', () => {
      const error = [
        ...
        },
        {
          ...
          result: {
            status: 'failed',
            reason:
              'Error entering parameters. Missing first parameter: length',
          },
        },
        {
      ...
      error.forEach(el => {
        it(el.description, () => {
          assert.deepEqual(checkParamTask6(...el.arguments), el.result);
        });
      });
    });
  });
